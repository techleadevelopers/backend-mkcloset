...
import {
  CorreiosResponse,
  ShippingOption,
} from './interfaces/correios.interface'; // Crie esta interface
...
export class ShippingService {
  ...
  async calculateShipping(
    ...
    if (!zipCode || zipCode.replace(/\D/g, '').length !== 8) {
      throw new BadRequestException(
        'CEP inválido. Por favor, digite um CEP válido com 8 dígitos.',
      );
    }
  ...
  private getTotalPackageInfo(items: CartItemForShipping[]) {
    ...
    items.forEach((item) => {
      const productWeight =
        item.product.weight || AppConstants.DEFAULT_PRODUCT_WEIGHT_KG;
      const productLength =
        item.product.dimensions?.length ||
        AppConstants.DEFAULT_PRODUCT_DIMENSION_CM.length;
      const productWidth =
        item.product.dimensions?.width ||
        AppConstants.DEFAULT_PRODUCT_DIMENSION_CM.width;
      const productHeight =
        item.product.dimensions?.height ||
        AppConstants.DEFAULT_PRODUCT_DIMENSION_CM.height;

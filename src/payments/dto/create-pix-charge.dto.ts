...
export class CreatePixChargeDto {
  @ApiProperty({
    description: 'ID do pedido relacionado a esta cobrança',
  ...
  @ApiPropertyOptional({
    description:
      'ID do convidado, se a compra for de um usuário não autenticado',
    example: 'uuid-do-convidado',
  })
  @IsUUID()
  @IsOptional()
...
export class PixChargeResponseDto {
  @ApiProperty({
    description: 'ID da transação gerada',
  ...
  @ApiProperty({
    ...
    example: 'https://api.example.com/pix/qrcode/uuid-da-transacao.png',
  })
  @IsString()
  @IsNotEmpty()
  qrCodeImage: string;
  ...
  @ApiPropertyOptional({
    description:
      'Mensagem de erro do BR Code (se houver, para depuração/exibição)',
    example: 'Código PIX inválido',
  })
  @IsOptional()
  @IsString()
  brCodeError?: string;
  ...
  @ApiPropertyOptional({
    description:
      'Data de expiração em formato alternativo (se usado internamente)',
    example: '2025-06-01T10:30:00.000Z',
  })
  @IsOptional()
  @IsString()
